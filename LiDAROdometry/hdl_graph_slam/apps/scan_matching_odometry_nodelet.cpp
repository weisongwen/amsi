...
private:
  ...
  void cloud_callback(const sensor_msgs::PointCloud2ConstPtr& cloud_msg) {
    ...
    Eigen::Matrix4f pose = matching(cloud_msg->header.stamp, cloud); // input time and cloud
  ...
  Eigen::Matrix4f matching(const ros::Time& stamp, const pcl::PointCloud<PointT>::ConstPtr& cloud) {
    if(!keyframe) { //  for example: for the first frame
    ...
    if(!registration->hasConverged()) { // if not converge, treat as constant velocity model
