...
*/
Eigen::MatrixXd shadowMatching::llh2ecef(Eigen::MatrixXd data) // transform the llh to ecef
{
...
output: llh (Matrix3d)
*/
Eigen::MatrixXd shadowMatching::ecef2llh(Eigen::MatrixXd data) // transform the ecef to llh
{
...
output: enu (Matrix3d)
*/
Eigen::MatrixXd shadowMatching::ecef2enu(Eigen::MatrixXd originllh, Eigen::MatrixXd ecef) // transform the ecef to enu 
{
	double pi = 3.1415926; // pi 
...
output: enu (Matrix3d)
*/
Eigen::MatrixXd shadowMatching::enu2ecef(Eigen::MatrixXd originllh, Eigen::MatrixXd enu) // transform the ecef to enu 
{
	// enu to ecef
	double  e = enu(0);
	double  n = enu(1);
...
*/
bool shadowMatching::readSkymask(string filepath) //  readSkymask from CSV file 
{
...
*/
void shadowMatching::string2llh(vector<string> poseReInsi,  vector<string> skysmaskString_)// extract the llh from the string for each building
{
...
output: the skymask
*/
shadowMatching::skyMask shadowMatching::preparingSkymaskbyPose(ENU ENU_)// extract the llh from the string for each building
{
...
// (PRN, EL, AZ)
*/
vector<int> shadowMatching::visibilityCal(MatrixXd eELAZAngles, shadowMatching::skyMask skyMask_) // calculate the expected visibility
{
	vector<int> result;
...
output: vector<int>
*/
double shadowMatching::scoreCal(MatrixXd eAllMeasurement, vector<int> visibility) // calculate the scores of shadowmatching
{
	double  score =0;
...
resol: 2m 
*/
vector<shadowMatching::particle> shadowMatching::generateParticles(Eigen::MatrixXd WLSECEF, double radis, double resol, Eigen::MatrixXd eAllMeasurement, Eigen::MatrixXd eELAZAngles) // Generate particle near the WLS solution
{
...
resol: 2m
*/
MatrixXd shadowMatching::weightSM_() // weight all the particles and obtain the final solution
{
	//
	Eigen::MatrixXd llh; // 
	llh.resize(3, 1); // 
	...
	if (particles.size() > 0) // do only when there at least one particle 
	{
		...
		for (int index = 0; index < particles.size(); index++) // index all the particles to nromalize 
		{
			particles[index].scores = particles[index].scores / particle_.scores;
			if (particles[index].scores > scoreThres)
			{
				EList.push_back(particles[index].E);
				NList.push_back(particles[index].N);
			}	
		}
		Eigen::MatrixXd enu; // 
		WSMenu_ = enu;
		...
		{
			enu(0) = enu(0) + EList[i] / EList.size();
			enu(1) = enu(1) + NList[i] / EList.size();
			enu(2) = 0;
		}
...
MatrixXd shadowMatching::MajorAxisCor(MatrixXd ENU1, MatrixXd eELAZAngles, Eigen::MatrixXd WLSECEF) // solution correction based on major axis 
{
	// 
	MatrixXd result;
	result.resize(3,1);
	skyMask skymask_;
	vector<particle> Majorparticles_;
	vector<int> Evisibility;
	double radis = 50;
	ENU ENU_;
	ENU_.E = ENU1(0);
	ENU_.N = ENU1(1);
	...
	skymask_ = preparingSkymaskbyPose(ENU_);
	if (skymask_.aziElemask.size() > 0)
		Evisibility = visibilityCal(eELAZAngles, skymask_); // calculate the expected visibility at each position
	for (double N = ENU1(1) - radis; N <= ENU1(1) + radis; N = N + map_.disReso) // search in the by line (N direction)
	{
		for (double E = ENU1(0) - radis; E <= ENU1(0) + radis; E = E + map_.disReso) // search in the by line (E direction)
		{
			particle particle_; // one particle
			double dis = sqrt((E - ENU1(0)) * (E - ENU1(0)) + (N - ENU1(1)) * (N - ENU1(1)));
			if (dis < radis) // in circle
			{
				ENU ENU_;
				ENU_.E = E;
				...
				skymask_ = preparingSkymaskbyPose(ENU_);
				if (skymask_.aziElemask.size()>0)
					visibility = visibilityCal(eELAZAngles, skymask_); // calculate the expected visibility at each position (eELAZAngles: empheris)
				particle_.eELAZAngles = eELAZAngles;
				particle_.visibility = visibility;
				double score = MajorAxisScoreCal(Evisibility, visibility);
				particle_.scores = MajorAxisScoreCal(Evisibility, visibility);
				// eAllMeasurement -> (n,3) PRN CNO Pseudorange
				// eELAZAngles -> // (PRN, EL, AZ)
				...
				cout << "finish one hypothesis (particle)-------------------"<< ENU_.E <<"      "<< ENU_.N << "      "<< "  visibility.size()"<< visibility.size() <<"\n";*/
				if (visibility.size()) // at least one satellite available
					Majorparticles_.push_back(particle_);
			}
		}
...
double shadowMatching::MajorAxisScoreCal(vector<int>Evisibility, vector<int> visibility) // calculate score (major axis)
{
	double  score = 0;
	if (visibility.size()) // at least one satellite 
	{
		for (int index = 0; index < Evisibility.size(); index++)
		{
			if (std::find(visibility.begin(), visibility.end(), Evisibility[index]) != visibility.end())
			{
				score++;
			}
		}
