...
public:
  ...
  pcl::PointCloud<PointT>::Ptr correct(const pcl::PointCloud<PointT>::ConstPtr& cloud ,
    ...
    /**************ndt registration and get estimated transformation***************
      The NDT-based registration is reserved here. As this NDT is time consuming. The NDT
      in Autoware is employed to provide mapping positioning (the pose_msg).
    */
    /*
    ...
    */
    ...
    std::cout<<pose_msg.pose.position.x<<std::endl;
    

    Eigen::Vector3f p = trans.block<3, 1>(0, 3);
    p(0,0)= pose_msg.pose.position.x;
    p(1,0)= pose_msg.pose.position.y;
    p(2,0)= pose_msg.pose.position.z;
